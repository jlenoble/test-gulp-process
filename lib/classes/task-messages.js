...
var _parallelMessages2 = _interopRequireDefault(_parallelMessages);
...
class TaskMessages {
  ...
  async next(results) {
    if (this.parallelMessages) {
      this.currentParallelMessages.push(...this.parallelMessages.next(this.message));
    }
    ...
    if (typeof value === 'string') {
      ...
      this.currentParallelMessages.push(...value.filter(fn => typeof fn !== 'function'));
      return this.next(results);
    } else if (typeof value === 'function') {
      this.globalFns.push(value);
      return this.next(results);
    } else if (value instanceof _parallelMessages2.default) {
    ...
    return !message.done && (await (0, _helpers.waitForMessage)(results, this.message));
  }
  ...
  async nextParallel(results) {
    ...
    if (this.debug) {
      console.info(`${_chalk2.default.cyan('Waiting for')} message '${_chalk2.default.green(searchedMessage)}'`);
    }

    await (0, _helpers.waitForMessage)(results, searchedMessage);

    const indices = this.currentParallelMessages.map(msg => {
      const _msg = msg.replace(/([.?*+^$[\]\\(){}|-])/g, '\\$1');
      const index = results.allMessages.findIndex(el => el.match(new RegExp(_msg)));
      if (index === -1) {
        return null;
      }
