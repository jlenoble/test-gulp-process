...
const newDest = exports.newDest = () => {
  counter++;
  return `/tmp/${_path2.default.basename(process.cwd())}_${Date.now()}_${counter}`;
};

const copySources = exports.copySources = options => {
...
const copyGulpfile = exports.copyGulpfile = options => {
  return new Promise((resolve, reject) => {
...
const copyBabelrc = exports.copyBabelrc = options => {
  return new Promise((resolve, reject) => {
    _gulp2.default.src('.babelrc').on('end', resolve).on('error', reject).pipe(_gulp2.default.dest(options.dest));
  });
};

const linkNodeModules = exports.linkNodeModules = options => {
